...
* 
* DFS를 이용해서 벽을 3개 심는다. 
* 벽을 다 심으면, BFS를 이용해서 바이러스를 퍼트린다.
* 바이러스가 퍼질 때 배열은 복사한 배열을 사용한다.
* BFS에 입력으로 주어졌던 바이러스의 위치들을 담는다. -> 굳이 매번 바이러스의 위치를 구할 필요가 없어서 입력 받을 때 List에 담았다.
* 바이러스는 상하좌우로 퍼지고, 바이러스로 변한 칸도 바이러스를 옮길 수 있다. -> 큐에 넣어준다.
* 
* 시간 872ms
* 메모리 295096KB
*/
